...
const Squares: React.FC<SquareProps> = ({
  ...
  const themeColors =
    theme === 'dark'
      ? { ...defaultDarkColors, ...darkMode }
      : { ...defaultLightColors, ...lightMode };
  ...
  useEffect(() => {
    ...
    const drawGrid = () => {
      ...
      if (themeColors.backgroundOpacity && themeColors.backgroundOpacity > 0) {
        ctx.fillStyle = `rgba(${hexToRgb(themeColors.backgroundColor)}, ${
      ...
      gradient.addColorStop(
        0,
        `rgba(${hexToRgb(themeColors.gradientStartColor)}, ${gradientStartOpacity})`
      );
      gradient.addColorStop(
        1,
        `rgba(${hexToRgb(themeColors.gradientEndColor)}, ${gradientEndOpacity})`
      );
